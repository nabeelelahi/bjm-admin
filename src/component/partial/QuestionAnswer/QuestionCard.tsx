...
function QuestionCard(props: QuestionAnswerDto & { isActive: boolean; setSelectedQuestion: Dispatch<SetStateAction<QuestionAnswerDto | null>> }) {
    return (
        <li
            ...
            className={`p-2 cursor-pointer rounded ${props.isActive ? "border-1 border border-solid border-[#0B6990]" : "hover:bg-gray-200"}`}
            onClick={() => props.setSelectedQuestion(props)}
        >
            <h3 className={`font-semibold text-md ${props.isActive ? "text-[#0B6990]" : ""}`}>{props.title as string}</h3>
